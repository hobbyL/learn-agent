...
## Session 19: 实现 13-observability：三层可观测性 + 双 Agent 对比 + LangSmith 可选降级

**Date**: 2026-07-10
**Task**: 实现 13-observability：三层可观测性 + 双 Agent 对比 + LangSmith 可选降级
**Branch**: `main`

### Summary

三层可观测性：LangSmith tracing（零代码 vs wrap_openai）+ eval（规则匹配 + LLM-as-Judge，8题数据集）+ 自定义 AgentTracer（终端+JSON输出）。双Agent对比（LangGraph vs 手写ReAct）。修正阶段归属（13从阶段3移回阶段2），阶段2全部完成9/9。
...
## Session 20: 实现 14-mcp-tool-discovery：MCP 动态工具发现 + 三阶段 demo + 多 Server 聚合

**Date**: 2026-07-11
**Task**: 实现 14-mcp-tool-discovery：MCP 动态工具发现 + 三阶段 demo + 多 Server 聚合
**Branch**: `main`

### Summary

MCP 动态工具发现：3 个 MCP Server（FastMCP+@mcp.tool+stdio）+ MCPClient 多 Server 聚合（AsyncExitStack 生命周期）+ schema 转换 + 工具路由。三阶段 demo（静态基线/MCP 动态/运行时热加载）端到端通过。阶段 3 开始，14/19 完成。
