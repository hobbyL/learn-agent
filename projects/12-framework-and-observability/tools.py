"""
工具层 —— 双接口：原生 execute_tool + LangChain @tool
=======================================================

本模块在项目 03 tools.py 的基础上做了一层适配：

1. 保留原有接口（v1 手写 ReAct 使用）：
   - execute_tool(tool_name, args) → str
   - TOOLS_SCHEMA（OpenAI Function Calling 格式）
   - get_tool_descriptions()

2. 新增 LangChain @tool 版本（v2/v3 使用）：
   - search_tool / lookup_tool / calculate_tool / compare_tool
   - lc_tools 列表：传给 ChatOpenAI.bind_tools() 和 ToolNode

设计原则：
    @tool 装饰器版本内部直接调用原始函数（search/lookup/calculate/compare），
    不重复实现业务逻辑，保持 DRY。
"""
...
def get_tool_descriptions() -> str:
    """生成工具描述文本，用于注入到 v1 Agent 的 system prompt 中。"""
    lines = ["你可以使用以下工具：", ""]
