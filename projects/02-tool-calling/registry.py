...
def tool(
    ...
    用法（两种都支持）：
        ...
        @tool(
            ...
            model=DiceRollerParams,  # Pydantic 模式下用这个模型
        )
        def dice_roller(...): ...
    ...
    关于 model：
        传入 Pydantic BaseModel 类。当 SCHEMA_ENGINE=pydantic 时，
        Schema 由 model.model_json_schema() 生成，取代手搓版的 inspect 流程。
        当 SCHEMA_ENGINE=handcraft 时，model 参数被忽略，依然走手搓路径。

    --------------------------------------------------------------
    SCHEMA_ENGINE 配置决定走哪条路线
    --------------------------------------------------------------
    - "handcraft"（默认）：手搓版，build_schema() 用 inspect 读函数签名生成 Schema
    - "pydantic"：Pydantic 版，build_schema_pydantic() 用 model_json_schema() 生成 Schema

    两条路线生成的 Schema 格式一样（都是 OpenAI tools 格式），只是生成方式不同。
    agent 不感知差异，只管从 registry 拿 Schema 用。
    ...
    def decorator(func: Callable) -> Callable:
        ...
        # 根据 SCHEMA_ENGINE 配置选择 Schema 生成方式
        if engine == "pydantic" and model is not None:
            # Pydantic 路线：model_json_schema() 一行生成
            from schema_gen_pydantic import build_schema_pydantic
            schema = build_schema_pydantic(tool_name, tool_desc, model)
        else:
            # 手搓路线（默认）：inspect + 类型映射
