...
### 阶段 2：Agent 系统设计（4-6周）

| 编号 | 项目名称 | 描述 | 状态 | 完成时间 |
|------|---------|------|------|---------|
| 05 | streaming-agent | 流式输出：streaming API + tool_calls 分块拼接 + 实时展示推理过程 | ⏳ 未开始 | — |
| 06 | short-term-memory | 短期记忆：对话历史管理 + context 压缩 + 摘要策略 | ⏳ 未开始 | — |
| 07 | long-term-memory | 长期记忆：向量化存储 + 语义检索 + 跨 session 记忆 | ⏳ 未开始 | — |
| 08 | structured-output-and-planning | 结构化输出 + 任务分解与目标树 | ⏳ 未开始 | — |
| 09 | hitl-agent | Human-in-the-Loop：主动中断 + 人类反馈 + 恢复执行 | ⏳ 未开始 | — |
| 10 | framework-and-observability | 框架对照（LangGraph / OpenAI Agents SDK）+ tracing/eval | ⏳ 未开始 | — |

### 阶段 3：多 Agent 系统（3-4周）

| 编号 | 项目名称 | 描述 | 状态 | 完成时间 |
|------|---------|------|------|---------|
| 11 | mcp-tool-discovery | MCP 动态工具发现：运行时工具绑定 vs 静态注册表 | ⏳ 未开始 | — |
| 12 | multi-agent-debate | 辩论模式：多 Agent 投票决策 | ⏳ 未开始 | — |
| 13 | code-review-team | 代码审查团队（分工协作） | ⏳ 未开始 | — |
| 14 | workflow-orchestration | 工作流编排引擎 | ⏳ 未开始 | — |
...
- ~~**流式输出 (streaming)**~~ → ✅ 独立为 05-streaming-agent（阶段 2 第一项）
- ~~**MCP 动态工具发现**~~ → ✅ 落地为 11-mcp-tool-discovery（阶段 3 开篇）
- ~~**框架对照节点**~~ → ✅ 落地为 10-framework-and-observability（含 tracing/eval）

> 原则：阶段 2 规划已锁定（6个项目），阶段 3 已调整（4个项目含MCP）。下一步直接开始 05-streaming-agent。
